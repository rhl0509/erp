...
| | |
|---|---|
| 무엇 | 회계·재고·발주/수주까지 다루는 ERP 백엔드 + 웹 관리 콘솔 |
| 스택 | FastAPI · SQLAlchemy · MySQL · Next.js |
| 규모 | Python 11,541줄 · TypeScript 19,390줄 · API 엔드포인트 100개 · 테이블 31개 |
| 스키마 이력 | 마이그레이션 16개 |
| 테스트 | 테스트 함수 130개 (7개 파일) |
| 진행 | 1인. DB 설계 → REST API → 화면 → 인증 흐름 전부 |

설계에서 신경 쓴 지점 세 가지:

- **채번을 행 잠금으로 처리**했다. 동시 요청에 전표번호가 겹치면 회계에서는 복구가 어렵다.
- **감사 로그에 변경 전후 값을 남긴다.** "누가 언제 무엇을 바꿨나"가 아니라 "무엇에서 무엇으로 바꿨나"까지 남아야 추적이 된다.
- 인증은 httpOnly 쿠키 JWT + RBAC. 2단계 인증(TOTP)·비밀번호 정책·무차별 대입 방어·세션 무효화 포함.
