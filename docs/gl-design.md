...
### 기간 마감 (2026-07-14 구현 — 설계 범위 밖이었으나 추가)

월 단위 회계기간(`accounting_periods`). 마감하면 **마감선(가장 최근 마감월) 이하의 모든 기간**이
얼어붙는다 — 수기 전표·과거 날짜 결제(`pay_date`)·원천(재고이동·결제) 삭제·재전기(rebuild)가
전부 400 `period_closed` 로 막힌다. 강제 지점은 `app/gl.py`의 `assert_period_open` 하나이며,
모든 전기가 `post_journal` 을, 모든 전표 삭제가 `delete_for_source` 를 지나므로 우회 경로가 없다.

- **마감 대체분개**: 그 달의 손익계정 잔액을 상계하고 차액을 3110 이월이익잉여금으로 넘기는
  `CLOSING` 전표(`source_id` = 기간 id, UNIQUE 로 기간당 1건). 전표일자는 그 달 말일.
- **진행 중인 달은 마감 불가** — 마감하면 그 달의 남은 영업(재고 입출고·결제)이 전부 막힌다.
  마감은 오래된 달부터 순서대로, 해제는 최근 마감분부터 역순으로만.
- **재무제표**: 손익계산서는 CLOSING 을 **제외**(마감했다고 그 달 매출이 0 이 되면 안 된다),
  재무상태표는 **포함**(마감분 이익은 자본으로 넘어가고 `net_income` 에는 미마감분만 남는다).
- **동시성**: 기표는 기간 행을 FOR SHARE, 마감/해제는 FOR UPDATE 로 잡아 직렬화한다
  (마감 커밋과 기표 커밋이 교차해 마감된 달에 전표가 끼어드는 경합 차단).
- **감사**: 마감·해제가 AuditLog(`entity=gl_period`)에 남고, 해제는 삭제되는 마감 전표의
  스냅샷(전표번호·순이익·라인)을 `before` 에 기록한다.
- 미구현: 연 결산(회계연도) 개념, 마감 후 정정을 위한 '당기 수정분개' 관행.

### 시간대 (2026-07-14 — 회계 날짜의 단일 소스)

전표일자·발행일·채번 기간·"이번 달"은 전부 `app/timeutil` 에서만 만든다.
- `BUSINESS_TZ`(기본 Asia/Seoul): 사업장 시간대 — 회계 날짜의 기준.
- `DB_TZ`(기본 UTC): DB 의 naive 타임스탬프(`created_at`)를 어느 시간대로 해석할지.

`created_at`(DB 시각)을 그대로 전표일자로 쓰면 **월 경계에서 하루가 인접 월로 샌다** —
UTC DB + KST 사업장이면 8/1 08:00 KST 출고가 `created_at=2026-07-31 23:00` 이라 7월 매출로
기표되고, 7월이 마감돼 있으면 그 정상 거래가 `period_closed` 로 거부된다.
`business_date_of()` 가 DB 시각 → 사업장 날짜 변환을 담당하며, 통계·리포트의 기간 경계는
`to_db_naive()` 로 반대 방향 변환을 쓴다(사업장 1일 0시 → DB 시각).
검증: `tests/test_timezone.py`. DB 를 KST 로 운영하면 `DB_TZ=Asia/Seoul` 로 두면 항등이 된다.
