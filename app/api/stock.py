...
from ..models import StockMovement, StockBalance, Item, Partner, User
from ..schemas import StockMovementCreate, StockMovementOut, StockLevelOut, Page
from ..deps import require_permission
from ..services import (
    record_audit, paginate, post_movement, apply_stock_delta, _signed_delta, StockError,
)
...
def _on_hand_sq():
    """품목별 현재고(잔고 캐시)를 읽는 상관 스칼라 서브쿼리. 잔고 행이 없으면 0."""
    return (
        select(func.coalesce(StockBalance.on_hand, 0))
...
def _to_out(m: StockMovement) -> StockMovementOut:
    """파생 필드(품목/거래처 이름)를 포함한 응답 스키마로 변환한다."""
...
def list_movements(
    ...
    movement_type: str | None = Query(None, description="IN/OUT/ADJUST 로 필터"),
...
def delete_movement(
    ...
    # 잔고 원복: 원래 증감의 반대를 적용. 삭제로 재고가 음수가 되거나 이동평균이
    # 음수로 오염되면 막는다. IN 삭제 시엔 매입단가를 넘겨 평균에서 해당 입고분 제거.
    try:
        apply_stock_delta(
...
def _with_on_hand(db: Session, items: list[Item]):
    """품목 목록에 잔고 캐시(현재고)를 매핑한다. 잔고 행이 없으면 0."""
    ids = [it.id for it in items]
    balances: dict[int, int] = {}
    if ids:
        rows = db.execute(
            select(StockBalance.item_id, StockBalance.on_hand).where(StockBalance.item_id.in_(ids))
        ).all()
        balances = {iid: int(v) for iid, v in rows}
